...
const authenticator = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!jwtToken) {
        const error: ErrorObject = {message: 'Invalid request, access token missing', code: 404}
        return next(error)
    }
    ...
    if (data) {
        ...
        const err: ErrorObject = {message: 'Access token failed to authenticate', code: 400}
        return next(err)
    }
    next()
}
