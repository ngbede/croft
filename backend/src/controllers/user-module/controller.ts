...
export default class UserController extends BaseController {
    ...
    async create (req: Request, res: Response, next: NextFunction, schema: ObjectSchema) {
        const newUser: user = req.body
        // validate request body
        const valid: boolean = this._validateBody(newUser, next, schema)

        if (valid) {
            const { user, error } = await this.api.auth.api
            .createUser({
                email: newUser.email,
            ...
            await this.api.auth.api.sendMagicLinkEmail(newUser.email, {shouldCreateUser: false})
    ...
    async passwordReset (req: Request, res: Response, next: NextFunction) {
        ...
        if (isEmail) {
            ...
            if (exist) {
                const { error } = await this.api.auth.api.resetPasswordForEmail(userObject.email)
                if (error) {
                    const err: ErrorObject = {message: error.message, code: error.status}
                    return next(err)
                }
